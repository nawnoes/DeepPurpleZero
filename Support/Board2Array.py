...
class Board2Array:
    ...
    def board2arrayFor41Feature(self,chessBoard,preMoves):
        ...
        boardArray.append(self.arrivalPositionOfMove(preMoves[0]))
        boardArray.append(self.arrivalPositionOfMove(preMoves[1]))
        boardArray.append(self.arrivalPositionOfMove(preMoves[2]))
        boardArray.append(self.arrivalPositionOfMove(preMoves[3]))
        boardArray.append(self.arrivalPositionOfMove(preMoves[4]))
    ...
    def board2array(self, chessBoard):
        ...
        return boardArray #, boardArray2
    #feature만들기
    def arrivalPositionOfMove(self,move):
        feature = [[0] * 8 for i in range(8)]
        if move == "None":
            return feature
        else:
            move = move[2:]
            moveIndex = chess.SQUARE_NAMES.index(move)
            k = 0
            for i in range(8):
                for j in range(8):
                    if k == moveIndex:  # 대문자로 흰색이면
                        feature[i][j] = 1
                        feature.reverse()
                        return feature
                    k += 1
